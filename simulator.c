...
int main() {
  ...
  int      cellColors[7] = {0xFF8822 /*Orange*/, 0xFF2222 /*Red*/, 0xFFFF44 /*Yellow*/,
			    ...
			    0x999999 /*LightGray*/};
  pthread_t display;
  pthread_t tower[7]; // array of tower thteads
  ...
  for (int i=0; i<ottawa.numTowers; i++) {
    ...
    pthread_create(&tower[i], NULL, handleIncomingRequests, &ottawa.towers[i]); // create thread for tower
  }

  // wait for their completion before exiting gracefully with some kind of message
  pthread_create(&display, NULL, showSimulation, &ottawa); // thread for display

  for (int i=0; i<ottawa.numTowers; i++) {
    pthread_join(tower[i], NULL); // wait for each tower to end
    printf("Tower %d thread is back.\n", i);
  }
  printf("All threads are back. Exiting.\n"); // donezo
